...
const API_CONTACT_URL = `${API_BASE_URL}/contact`;
...
function Kontak() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Mengirim data ke endpoint /api/contact
            ...
            setNotificationMessage({ type: 'error', text: msg });
            setShowNotification(true);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="kontak-page-container"> 
                    ...
                    <div className="contact-form-section">
                        <form className="contact-form-card" onSubmit={handleSubmit}> 
                            ...
                            <div className="form-group">
                                ...
                                <input id="name" className="form-input" type="text" name="name" value={formData.name} onChange={handleChange} placeholder="Masukkan Nama Lengkap" required />
                            </div>
                            <div className="form-group">
                                <label htmlFor="email" className="form-label">Masukkan Email</label>
                                <input id="email" className="form-input" type="email" name="email" value={formData.email} onChange={handleChange} placeholder="Masukkan Email" required />
                            </div>
                            <div className="form-group">
                                <label htmlFor="phone" className="form-label">No. Telepon</label>
                                <input id="phone" className="form-input" type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="Masukkan No. Telepon (Opsional)" />
                            </div>
                            <div className="form-group">
                                <label htmlFor="message" className="form-label">Pesan Anda</label>
                                <textarea id="message" className="form-textarea" name="message" value={formData.message} onChange={handleChange} placeholder="Tulis pesan Anda di sini..." rows="5" required disabled={loading}></textarea>
                            </div>
            ...
            {/* Footer Placeholder tetatp sama */}
        </div>
    );
}
